...
### 메모
 ...
 <p>처음 문제를 풀 때 커서를 인덱스로 직접 관리했는데, 이렇게 하면 매번 해당 위치까지 이동해야 해서 구현이 복잡하고 불필요한 비용이 생겼었다 <br>
  이후엔 Iterator를 사용하여 현재 위치를 직접 참조한 상태에서 이동·삽입·삭제를 수행할 수 있어 코드가 더 간결해졌고, 커서 관리 부담도 줄어들어 문제의 의도에 더 적합하게 해결할 수 있었다</p>
